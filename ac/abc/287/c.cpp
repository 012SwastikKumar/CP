...
void dfs(ll n, vll &vis, vll &dfsTraversal){
	// dfsTraversal.push_back(n);
	vis[n]=1;

	for(auto &it : adj[n]){
		if(!vis[it]){
			dfs(it,vis,dfsTraversal);
		}
	}
}

bool check(ll n, vll &vis1, ll prev){
	vis1[n]=1;
	for(auto &it : adj[n]){
		if(!vis1[it]){
			if(check(it,vis1,n))return true;
...
void solve()
{
	...
	vll vis(n+1),dfsTraversal;

	// if(!vis[1]){
		dfs(1,vis,dfsTraversal);
	// }
